...
- (void)viewDidLoad {
    [super viewDidLoad];
    tf_bank = [[UITextField alloc] initWithFrame:CGRectMake(50, 50, 300, 40)];;
    tf_bank.delegate =self;
    tf_bank.backgroundColor = [UIColor orangeColor];
    tf_bank.placeholder = @"请输入银行卡卡号";
    [tf_bank addTarget:self action:@selector(dd:) forControlEvents:UIControlEventEditingChanged];
    [self.view addSubview:tf_bank];
    lable = [[UILabel alloc] initWithFrame:CGRectMake(5, 100, 400, 50)];
...
-(void)dd:(NSString *)text{
    NSString *bankNumber = @"";
    bankNumber = [tf_bank.text stringByReplacingOccurrencesOfString:@" " withString:@""];
    if (tf_bank.text.length < 5) {
        lable.text = @"";
    }
    if (bankNumber.length == 8) {
       
        lable.text = [self backbankenameWithBanknumber:bankNumber];
    }
